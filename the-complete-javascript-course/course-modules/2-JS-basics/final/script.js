...
console.log('operator lt', johnOlder);
...
var ageJack = calculateAge(1986);
console.log(ageJohn, ageMike, ageJane, ageJack);
...
yearsUntilRetirement(1986, 'Jack');
*/
...
var tips = [];
tips.push(calcTip(paid[0]));
tips.push(calcTip(paid[1]));
tips.push(calcTip(paid[2]));
console.log('Tips', tips);

var paids = [];
paids.push(calcPaid(paid[0]));
paids.push(calcPaid(paid[1]));
paids.push(calcPaid(paid[2]));
...
var higherBMI = mark.calcBmi() > john.calcBmi() ? mark.fullName() +' '+ parseInt(mark.bmi) : john.fullName() +' '+ parseInt(john.bmi);
console.log('The Higher BMI is: '+ higherBMI);
*/
...
John likes to tip 20% of the bill when the bill
is less than $50, 15% when the bill is between $50 and $200,
...
10% when the bill is between $100 and $300, and 25% if the bill
...
average of a given array of tips. HINT: Loop over the array,
...
var accounts = {
  ...
  paidTips: [],
  avgTips: 0,
  calcBills: function(bill) {
    return bill + this.calcTips(bill);
  },
  ...
  saveBills: function() {
    for (var i = 0; i < this.bills.length; i++) {
      var paidBill = this.calcBills(this.bills[i]);
      this.paidBills.push(paidBill);
    }
  },
  saveTips: function() {
    for (var i = 0; i < this.bills.length; i++) {
      var paidTip = this.calcTips(this.bills[i]);
      this.paidTips.push(paidTip);
    }
  },
  removeBillsTips: function(qtyRemove) {
    for (var i = 0; i < qtyRemove; i++) {
      this.paidBills.pop();
      this.paidTips.pop();
    }
  },
  calcAvgTips: function() {
    if (this.paidTips.length > 0) {
      for (var i = 0; i < this.paidTips.length; i++) {
        this.avgTips += this.paidTips[i];
      }
      this.avgTips /= this.paidTips.length;
    } else {
      alert("Calc paid tips first", this.paidTips);
    }
  },
  calcAll: function() {
    this.saveBills();
    ...
    this.calcAvgTips();
    console.log('calculating avg Tips...');
    console.log('Done.');
  },
}
